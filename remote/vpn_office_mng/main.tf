data "terraform_remote_state" "office" {
  ...
  config = {
    path = "../../office2/terraform.tfstate"
  }
*/
  backend = "s3"
...
data "terraform_remote_state" "mng" {
  ...
  config = {
    endpoint   = "storage.yandexcloud.net"
    bucket     = "oblanit-terraform-test"
    key        = "stend/remote/mng/base/terraform.tfstate"
    region     = "ru-central1"
    access_key = "YCAJEe8t4zC0J4rpHCEnxDLcW"
    secret_key = "YCO5JcIjLARdZozV8UUeI787O4gvmBJNfyJsf-jk"
...
resource "yandex_vpc_address" "remote_ext_ip" {
  ...
  external_ipv4_address {
    zone_id = data.terraform_remote_state.mng.outputs.zone
  }
}
...
module "vpn_tunnel" {
  ...
  local_dst_prefix = data.terraform_remote_state.mng.outputs.subnet_cidr[0]
  //local_nxt_hop =

  remote_vpc_id     = data.terraform_remote_state.mng.outputs.vpc_id
  ...
  remote_cloud_zone = data.terraform_remote_state.mng.outputs.zone
  ...
  remote_subnet_id        = data.terraform_remote_state.mng.outputs.subnet_id
}
